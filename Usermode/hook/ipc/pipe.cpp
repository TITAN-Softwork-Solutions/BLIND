...
namespace IXIPC
{
    ...
    static void PipeDebugLog(_In_z_ _Printf_format_string_ PCSTR format, ...) noexcept
    {
        ...
        char line[768]{};
        (void)StringCchPrintfA(line, RTL_NUMBER_OF(line), "[IXIPC pid=%lu tid=%lu] %s\n", GetCurrentProcessId(),
    ...
    static bool EnsurePipeOpenLocked(DWORD timeoutMs)
    {
        ...
        HANDLE hPipe = CreateFileW(hookPipeName, GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING,
    ...
    bool PublishHookEventSynchronously(const IXIPC_HOOK_EVENT &eventRecord) noexcept
    {
        ...
        bool ok =
            TransactCommandLocked(IxIpcCommandPublishHookEvent, &eventRecord, sizeof(eventRecord), nullptr);
        ReleaseSRWLockExclusive(&g_pipeLock);
        return ok;
    }
    ...
    bool RegisterProcessInstrumentationCallback(UINT64 callbackAddress, UINT64 callbackSize, UINT32 flags) noexcept
    {
        ...
        bool ok;
        AcquireSRWLockExclusive(&g_pipeLock);
        ok = TransactCommandLocked(IxIpcCommandRegisterProcessInstrumentationCallback, &request, sizeof(request),
                                   nullptr);
        ReleaseSRWLockExclusive(&g_pipeLock);
