...
namespace IC_STACKTRACE
{
    namespace
    {
        using RtlCaptureStackBackTraceFn = USHORT(WINAPI *)(ULONG FramesToSkip,
        ...
        using RtlLookupFunctionEntryFn = PRUNTIME_FUNCTION(WINAPI *)(DWORD64 ControlPc,
                                                                     PDWORD64 ImageBase,
        ...
        SymGetOptionsFn g_SymGetOptions = nullptr;
        ...
        bool EnsureDbgHelp() noexcept
        {
            if (g_SymGetOptions != nullptr && g_SymSetOptions != nullptr && g_SymInitialize != nullptr &&
                g_SymCleanup != nullptr && g_SymFromAddr != nullptr && g_SymGetLineFromAddr64 != nullptr)
            {
                return true;
            }
            ...
            if (dbghelp == nullptr)
            {
                ...
                auto loadLibraryW = kernel32 != nullptr
                                        ? reinterpret_cast<LoadLibraryWFn>(::GetProcAddress(kernel32, "LoadLibraryW"))
                                        : nullptr;
                dbghelp = loadLibraryW != nullptr ? loadLibraryW(L"dbghelp.dll") : nullptr;
            }
            if (dbghelp == nullptr)
            {
                return false;
            ...
            auto symGetOptions = reinterpret_cast<SymGetOptionsFn>(::GetProcAddress(dbghelp, "SymGetOptions"));
            auto symSetOptions = reinterpret_cast<SymSetOptionsFn>(::GetProcAddress(dbghelp, "SymSetOptions"));
            auto symInitialize = reinterpret_cast<SymInitializeFn>(::GetProcAddress(dbghelp, "SymInitialize"));
            auto symCleanup = reinterpret_cast<SymCleanupFn>(::GetProcAddress(dbghelp, "SymCleanup"));
            auto symFromAddr = reinterpret_cast<SymFromAddrFn>(::GetProcAddress(dbghelp, "SymFromAddr"));
            auto symGetLineFromAddr64 =
                reinterpret_cast<SymGetLineFromAddr64Fn>(::GetProcAddress(dbghelp, "SymGetLineFromAddr64"));
            if (symGetOptions == nullptr || symSetOptions == nullptr || symInitialize == nullptr ||
                symCleanup == nullptr || symFromAddr == nullptr || symGetLineFromAddr64 == nullptr)
            {
                return false;
            }

            g_DbgHelp = dbghelp;
            g_SymGetOptions = symGetOptions;
            g_SymSetOptions = symSetOptions;
            g_SymInitialize = symInitialize;
            g_SymCleanup = symCleanup;
            g_SymFromAddr = symFromAddr;
            g_SymGetLineFromAddr64 = symGetLineFromAddr64;
            return true;
        }
        ...
        static bool IsInternalInstrumentationModule(HMODULE mod) noexcept
        {
            ...
            static constexpr IX_RUNTIME_INTERNAL::IxEncodedWideLiteral kControllerName{L"BLINDController.exe", 0x20Bu};
            static constexpr IX_RUNTIME_INTERNAL::IxEncodedWideLiteral kInterfaceName{L"BLINDInterface.exe", 0x22Du};
        ...
        static void FillModuleInfo(void *ip,
                                   void *&moduleBaseOut,
                                   std::uint32_t &rvaOut,
    ...
    bool InitSymbols() noexcept
    {
        ...
        if (!g_SymInitialize(proc, nullptr, TRUE))
            return false;
    ...
    bool Resolve(const Trace &trace, ResolvedFrame *resolved, std::size_t resolvedCap) noexcept
    {
        ...
        for (std::uint16_t i = 0; i < trace.Count; ++i)
        {
            ...
            if (g_SymFromAddr(proc, reinterpret_cast<DWORD64>(rf.Ip), &displacement, sym))
            {
            ...
            DWORD lineDisp = 0;
            if (g_SymGetLineFromAddr64(proc, reinterpret_cast<DWORD64>(rf.Ip), &lineDisp, &line))
            {
    ...
    void InitCallerClassifier(void *anyFnInOwnModule) noexcept
    {
        ...
        ::GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                             reinterpret_cast<LPCSTR>(anyFnInOwnModule),
                             &mod);
        g_OwnModule = mod;
    }
    ...
    void SetAnalysisSubjectMetadata(std::uint32_t subjectKind,
                                    const wchar_t *subjectPath,
